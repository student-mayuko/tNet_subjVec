...
class SGD:
    ...
    def choice_vec_by_shrink_rate(self,word_info,word_vec):
        shrink_rate = []
        x_vec,y_vec = [],[]
        ...
        return torch.tensor(x_vec[shrink_rate.index(min(shrink_rate))],dtype=torch.float64,device=self.device),torch.tensor(y_vec[shrink_rate.index(min(shrink_rate))],dtype=torch.float64,device=self.device)

    def sum_calculate(self,word_info,word_vec,key):
    ...
    def fit(self, word_info, word_vec):
        ...
        while not(all(before_word_x == after_word_x) and all(before_word_y == after_word_y)):
            ...
            while before_loss != after_loss:
                ...
                self.grad = self.sum_calculate(self_word_info,self_word_vec,"grad")+2*(self.M*y-y)*y
                print('grad:',self.grad,',',type(self.grad))
                self.M -= self.eta * self.grad
                after_loss = self.loss
            #(x,y)の更新を行う
            x,y= self.choice_vec_by_shrink_rate(self_word_info,self_word_vec)    
