...
class Instructor:
    ...
    def _reset_params(self):
        #child→畳み込み層や全結合層、Bi-LSTM層とかの情報だね！
        for child in self.model.children():
#            print('-------------------')
#            print('対象：',child)
            if type(child) != BertModel:  # skip bert params
#                print('パラメータ内容：',child.state_dict().keys())
                for p in child.parameters():
#                    print("長さ：",len(p.shape),', パラメータ：',p)
                    if p.requires_grad:
                        #変更点(ここで初期化が行われる)
                        if len(p.shape) > 1:   #weight
    ...
    def run(self):
        ...
        '''
        〇_data_loader:
         ・text_bert_indices:tensor([[0,...,8],...,[2,...,3]])
         ・bert_segments_ids:同じ感じ
         ・text_raw_bert_indices:同じ感じ
         ・aspect_bert_indices:同じ感じ
         ・text_raw_indices:同じ感じ
         ・text_raw_without_aspect_indices:同じ感じ
         ・text_left_indices:同じ感じ
         ・text_left_without_aspect_indices:同じ感じ
         ・text_right_indices:同じ感じ
         ・text_right_with_aspect_indices:同じ感じ
         ・aspect_indices:
         ・aspect_in_text:tensor([[2,2],[6,7],..,[8,9]])
         ・polarity:tensor([2,1,.......,0])
        '''
        self._reset_params()
...
def main():
    ...
    --dropout 0.3
    --num_epoch 100
    --batch_size 64(laptop,twitter), 25(restaurant)
    --hidden_dim 50
    --l2reg  1e-4  (weight_decay, l2_penaltiy in Adam)
    --seed 14890(laptop,twitter), 11456(restaurant)
    '''
    ...
    #ratio:比、validation support:検証サポート
    ...
    input_colses = {
        ...
        'tnet_lf': ['text_raw_indices', 'aspect_indices', 'aspect_in_text'], #(原テキストインデックス,アスペクトインデックス,テキスト内のアスペクト)
